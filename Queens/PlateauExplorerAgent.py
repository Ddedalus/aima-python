...
class PlateauLimitedAgent:
	@staticmethod
	def new_agent(threshold):
		def count_collisions(board):
			count = 0
			queens = len(board)
			for i in range(queens):
		...
		def program(state):
			...
			if cols == 0:
				...
				update = check_actions(state, 10)
				if update['count'][0] < cols:
					program.plateau, program.count = None, 0
					return update, update['state'][0]
				...
				elif len(program.plateau) > 0 and program.count < threshold:  # there are options left to
					#  be explored
					ans = program.plateau['state'][0]
					program.plateau = program.plateau[1:]
					program.count += 1
